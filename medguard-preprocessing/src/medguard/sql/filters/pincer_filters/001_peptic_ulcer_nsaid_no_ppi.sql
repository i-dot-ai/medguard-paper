-- Filter 001: Patients with a history of peptic ulcer who have been prescribed a non-selective non-steroidal anti-inflammatory drug without co-prescription of a proton-pump inhibitor
--
...
-- 3. Do NOT have a co-prescribed proton pump inhibitor (PPI) during the NSAID prescription period
--
-- Design decisions:
...
-- - Co-prescription defined as: PPI prescription with overlapping dates (start/end dates overlap with NSAID dates)
-- - Looks for any peptic ulcer diagnosis in patient history
-- - Excludes selective COX-2 inhibitors (concept_id 387050005) as they are not "non-selective" NSAIDs
-- - Prioritizes precision: only flags clear cases of NSAID without PPI co-prescription
...
nsaid_codes AS (
    ...
    -- Based on concept_id 372665008 (Non-steroidal anti-inflammatory agent)
    -- IMPORTANT: This list includes UK SNOMED CT Extension codes (dm+d codes) for medication products
    -- UK dm+d codes are 13-15 digit codes containing '1000001' representing specific formulations
    -- Parent/international codes are included for reference but UK prescriptions use dm+d codes
    SELECT code FROM (VALUES
...
nsaid_without_ppi AS (
    -- Find NSAID prescriptions without overlapping PPI co-prescription
    SELECT DISTINCT
    ...
    WHERE NOT EXISTS (
        -- Check if there's any PPI prescription that overlaps with this NSAID prescription
        SELECT 1
        FROM patients_with_ppi ppi
        WHERE ppi.FK_Patient_Link_ID = nsaid.FK_Patient_Link_ID
            -- PPI overlaps with NSAID if:
            -- PPI starts before NSAID ends AND PPI ends after NSAID starts
            AND ppi.ppi_start_date <= nsaid.nsaid_end_date
            AND ppi.ppi_end_date >= nsaid.nsaid_start_date
    )
)
